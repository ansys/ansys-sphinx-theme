\renewcommand{\sphinxmaketitle}{%
    ...
    \hbox to \myHBoxWidth{%
        \vbox to \distanceToBottomOfAnsysBox{}%
        \hfil
        % draw the box
        \parbox[b]{\ansysBlockWidth}{%
            \raggedleft
    ...
    % we use a savebox so that we can draw the bottom material with respect
    % to the bottom of the page
    \sbox{\myTitle}{%
        \vbox{%
            % vskip skips down from the previous Ansys block and picture
            \vskip 1in
            % title
    ...
    \hbox to \hsize{%
        ...
        \hfil% pushes the right block to the right
        % vbox stacks the date and the ISO box
        \vbox{%
            \hbox{\pyvar{_date.strftime('%b %d, %Y').capitalize()}}
            \vskip 0.5\baselineskip
            % fbox wrapped in hbox to control width; vbox gives multiple lines
            \hbox{%
                \fbox{%
                    \vbox{%
                        \hbox{ANSYS, Inc. and}

"""
Optimized Flask Web Service for PyAnsys Documentation Navigator
Supports multiple libraries with REST and Socket.IO endpoints
"""
...
def initialize_chatbot(lib_name):
    ...
    def _init():
        ...
        if index is None:
            initialization_status[lib_name].update({
                'loading': False,
                'error': f"Index for {lib_name} not found",
                'status': 'Failed'
            })
            return
...
def get_chat_session(lib_name, session_id):
    ...
    chat_engine = CondensePlusContextChatEngine.from_defaults(
        ...
        3. If the question is not related to the {lib_name} library or there is no relative context, respond EXACTLY with: "Sorry i can only answer questions based on the provided documentation and codebase."
        and provide the docs link to the documentation
...
def handle_connect():
    ...
    if lib_name not in initialization_status:
        initialization_status[lib_name] = {'initialized': False, 'loading': False, 'error': None, 'status': 'Not initialized'}
